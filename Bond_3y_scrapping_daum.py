...
# 수집 대상 url
url = 'https://finance.daum.net/domestic/exchange/BOND-%2FKRTSY3Y%3DKQ'
driver = webdriver.Chrome(executable_path="./chromedriver.exe", chrome_options=options)
driver.get(url)

# 빈 데이터프레임 생성
output = pd.DataFrame(columns=['date', 'KTB3Y'])
...
for n in range(0, 81):
    ...
    # 테이블에서 날짜, KTB3Y금리 데이터 수집
    for i, dt in enumerate(dates):
        date = dt.text
        date = date.replace('.', '')
        CD91 = table[i][0].text
        df = pd.DataFrame([[date, CD91]], columns=['date', 'KTB3Y'])
...
output.to_csv('./raw_data/KTB3Y 금리.csv', index=False, encoding='cp949')
print(output)